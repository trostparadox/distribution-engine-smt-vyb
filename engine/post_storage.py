...
class PostsTrx(object):
    ...
    def get_discussions_by_created(self, token, tag=None, limit=100, last_timestamp=None, hive_select=None):
        ...
        q = "SELECT p.* FROM posts p INNER JOIN accounts acc ON p.author = acc.name AND p.token = acc.symbol WHERE p.muted = 'false' AND acc.muted = 'false' AND p.token = :token AND p.main_post = 'true' AND p.created > :cutoff %s %s %s ORDER BY created DESC LIMIT :limit" % (tag_clause, last_timestamp_clause, hive_select_clause)
    ...
    def get_discussions_by_blog(self, token, accounts, include_reblogs=False, last_timestamp=None, limit=100, hive_select=None):
        ...
        if include_reblogs:
            return self.db.query(f"SELECT index.reblogged_by, p2.* FROM (SELECT authorperm, reblogged_by, MIN(t) t FROM (SELECT r.account reblogged_by, p.authorperm, r.timestamp t FROM posts p INNER JOIN reblogs r ON p.authorperm = r.authorperm INNER JOIN accounts acc ON p.author = acc.name AND p.token = acc.symbol WHERE p.muted = 'false' AND acc.muted = 'false' AND token = :token AND main_post = 'true' AND r.account != p.author AND r.account in :accounts {cutoff_clause} {hive_select_clause} UNION SELECT NULL reblogged_by, p.authorperm, p.created t FROM posts p WHERE token = :token AND main_post = 'true' AND p.muted = 'false' AND p.author in :accounts {cutoff_clause} {hive_select_clause}) AS merged WHERE TRUE {reblog_timestamp_clause} GROUP BY authorperm, reblogged_by ORDER BY t desc LIMIT :limit) index INNER JOIN posts p2 ON p2.authorperm = index.authorperm AND p2.token = :token", token=token, accounts=tuple(accounts), last_timestamp=last_timestamp, limit=str(limit), cutoff=cutoff)
        else:
            return self.db.query(f"SELECT p.* FROM posts p INNER JOIN accounts acc ON p.author = acc.name AND p.token = acc.symbol WHERE p.muted = 'false' AND acc.muted = 'false' AND token = :token AND main_post = 'true' AND p.author in :accounts {cutoff_clause} {timestamp_clause} {hive_select_clause} ORDER BY p.created desc LIMIT :limit", token=token, accounts=tuple(accounts), last_timestamp=last_timestamp, limit=str(limit), cutoff=cutoff)
    ...
    def get_discussions_by_comments(self, token, accounts, last_timestamp=None, limit=100, hive_select=None):
        ...
        return self.db.query(("SELECT p.* FROM posts p INNER JOIN accounts acc ON p.author = acc.name AND p.token = acc.symbol WHERE p.muted = 'false' AND acc.muted = 'false' AND token = :token AND main_post = 'false' AND p.author in :accounts AND p.created > :cutoff %s %s ORDER BY p.created desc LIMIT :limit" % (timestamp_clause, hive_select_clause)), token=token, accounts=tuple(accounts), last_timestamp=last_timestamp, limit=str(limit), cutoff=cutoff)
    ...
    def get_feed_discussions(self, token, accounts, last_timestamp=None, limit=100, include_reblogs=True, hive_select=None):
        ...
        if include_reblogs:
            return self.db.query(("WITH following_table AS (SELECT following from follows where follower IN :accounts AND state = 1) SELECT index.accounts reblogged_by, p2.* FROM (SELECT authorperm, string_agg(account, ',') accounts, MIN(t) t FROM (SELECT p.authorperm, r.account, r.timestamp t FROM posts p INNER JOIN reblogs r ON p.authorperm = r.authorperm INNER JOIN accounts acc ON p.author = acc.name AND p.token = acc.symbol WHERE author NOT IN :accounts AND p.muted = 'false' AND acc.muted = 'false' AND token = :token AND main_post = 'true' AND p.created > :cutoff AND r.account in (SELECT * FROM following_table) UNION SELECT p.authorperm, NULL account, p.created t FROM posts p WHERE author NOT IN :accounts AND token = :token AND main_post = 'true' AND p.created > :cutoff AND p.muted = 'false' AND p.author in (SELECT * FROM following_table) ) AS merged %s %s GROUP BY authorperm ORDER BY t desc LIMIT :limit) index INNER JOIN posts p2 ON p2.authorperm = index.authorperm AND p2.token = :token" % (timestamp_clause, hive_select_clause)), token=token, accounts=tuple(accounts), last_timestamp=last_timestamp, limit=str(limit), cutoff=cutoff)
        else:
            return self.db.query(("WITH following_table AS (SELECT following from follows where follower IN :accounts AND state = 1) SELECT p.* FROM posts p INNER JOIN accounts acc ON p.author = acc.name AND p.token = acc.symbol WHERE p.author NOT IN :accounts AND p.muted = 'false' AND acc.muted = 'false' AND p.token = :token AND p.main_post = 'true' AND p.created > :cutoff AND p.author in (SELECT * FROM following_table) %s %s ORDER BY p.created desc LIMIT :limit" % (noreblog_timestamp_clause, hive_select_clause)), token=token, accounts=tuple(accounts), last_timestamp=last_timestamp, limit=str(limit), cutoff=cutoff)
    ...
    def get_discussions_by_score(self, score_key, token, tag=None, limit=100, last_authorperm=None, main_post=True, hive_select=None):
        ...
        q = f"SELECT p.* FROM posts p INNER JOIN accounts acc ON p.author = acc.name AND p.token = acc.symbol WHERE p.token = :token AND p.muted = 'false' AND acc.muted = 'false' AND p.main_post = :main_post AND p.created > :cutoff {tag_clause} {last_score_clause} {hive_select_clause} {extra_conditions} ORDER BY {score_key} DESC LIMIT :limit"
